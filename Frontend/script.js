...
function initMap() {
    map = new google.maps.Map(document.getElementById("map"), {
        ...
        minZoom: 15.5, maxZoom: 20, restriction: { latLngBounds: campusBounds, strictBounds: false },
    });
...
async function fetchMarkerData() {
    try {
        ...
        locationsData.forEach((place) => {
            ...
            allMarkers.push({ markerObject: marker, title: place.title.toLowerCase() });
        });
    } catch (error) {
...
function drawCampusPolygon() {
    ...
    const campusPolygon = new google.maps.Polygon({
        paths: campusBoundary, strokeColor: "#1b3899", strokeOpacity: 0.8,
        strokeWeight: 2, fillColor: "#448bef", fillOpacity: 0.05, map: map,
    });
